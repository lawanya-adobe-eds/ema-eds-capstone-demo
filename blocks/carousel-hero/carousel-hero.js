...
function updateActiveSlide(slide) {
  ...
  indicators.forEach((indicator, idx) => {
    ...
    if (idx !== slideIndex) {
      indicator.classList.remove('carousel-hero-slide-indicator-active');
      button.removeAttribute('disabled');
    } else {
      indicator.classList.add('carousel-hero-slide-indicator-active');
...
export default async function decorate(block) {
  ...
  rows.forEach((row, idx) => {
    ...
    slide.querySelectorAll('img').forEach((img) => {
      ...
      // Reserve layout space to prevent CLS. Slides render full-width at
      // 56.25vw (16:9) via object-fit; 1600x900 is a sizing hint only — no
      // visual change.
